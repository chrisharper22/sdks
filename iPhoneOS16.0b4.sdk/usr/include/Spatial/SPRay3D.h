...
/*!
 @abstract Returns a ray with a direction that's rotated by the specified quaternion.
...
SPRay3D SPRay3DRotateByQuaternion(SPRay3D ray, simd_quatd quaternion) {
    ...
    simd_double3 v = simd_act(simd_normalize(quaternion), ray.direction.vector);
    
    SPRay3D r = SPRay3DMake(ray.origin,
                            (SPVector3D) { .vector = v });
...
/*!
 @abstract Returns a ray with a direction that's rotated by the specified rotation.
