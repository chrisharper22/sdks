...
/*!
 ...
 @returns A new pose structure.
 @note if the upper-left 3 x 3 submatrix is not a rotation matrix (that is, orthogonal with a determinant of @p +1), the function returns @p SPPose3DInvalid .
 */
SPATIAL_INLINE
SPATIAL_OVERLOADABLE
...
SPPose3D SPPose3DMakeWithAffineTransform(SPAffineTransform3D transform) {
    ...
    SPPoint3D position = SPPoint3DMakeWithVector(SPAffineTransform3DGetOffset(transform));
...
SPPose3D SPPose3DMakeWithProjectiveTransform(SPProjectiveTransform3D transform) {
    ...
    SPPoint3D position = SPPoint3DMakeWithVector(SPProjectiveTransform3DGetOffset(transform));
